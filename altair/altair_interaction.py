...
@app.cell
def _(alt, cars):
    _selection = alt.selection_single()
    alt.Chart(cars).mark_circle().add_selection(_selection).encode(x='Horsepower:Q', y='Miles_per_Gallon:Q', color=alt.condition(_selection, 'Cylinders:O', alt.value('grey')), opacity=alt.condition(_selection, alt.value(0.8), alt.value(0.1)))
    return
...
def _(alt, cars):
    def plot(selection):
        return alt.Chart(cars).mark_circle().add_selection(_selection).encode(x='Horsepower:Q', y='Miles_per_Gallon:Q', color=alt.condition(_selection, 'Cylinders:O', alt.value('grey')), opacity=alt.condition(_selection, alt.value(0.8), alt.value(0.1))).properties(width=240, height=180)
...
def _(alt, genres, movies, mpaa):
    ...
    _selection = alt.selection_single(name='Select', fields=['Major_Genre', 'MPAA_Rating'], init={'Major_Genre': 'Drama', 'MPAA_Rating': 'R'}, bind={'Major_Genre': alt.binding_select(options=genres), 'MPAA_Rating': alt.binding_radio(options=mpaa)})
    # scatter plot, modify opacity based on selection
    alt.Chart(movies).mark_circle().add_selection(_selection).encode(x='Rotten_Tomatoes_Rating:Q', y='IMDB_Rating:Q', tooltip='Title:N', opacity=alt.condition(_selection, alt.value(0.75), alt.value(0.05)))
    return
...
def _(alt, movies):
    ...
    years = alt.Chart(movies).mark_bar().add_selection(_brush).encode(alt.X('year(Release_Date):T', title='Films by Release Year'), alt.Y('count():Q', title=None)).properties(width=650, height=50)  # limit selection to x-axis (year) values
...
def _(alt, sp500):
    ...
    alt.vconcat(_base.encode(alt.X('date:T', title=None, scale=alt.Scale(domain=_brush))), _base.add_selection(_brush).properties(height=60), data=sp500)
    return
...
def _(alt, movies):
    ...
    alt.layer(plot_1.add_selection(hover).add_selection(click), _base.mark_point(size=100, stroke='firebrick', strokeWidth=1), _base.mark_text(dx=4, dy=-8, align='right', stroke='white', strokeWidth=2).encode(text='Title:N'), _base.mark_text(dx=4, dy=-8, align='right').encode(text='Title:N'), data=movies).properties(width=600, height=450)
    return
