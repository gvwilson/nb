...
- `brooks`: Teon Brooks material for Memorial Sloan Kettering
- `chen`: Dan Chen material from UBC
- `davis`: intro CS classes from Neal Davis
...
- `kamvar`: Zhian Kamvar's R Markdown notebooks
...
-   `uv venv`, activate, and `uv sync`
-   `brew install eccodes` or equivalent before trying to run `kis/*.py` notebook.
-   `uv pip install -r dir/requirements.txt` to get tutorial-specific dependencies
